...
const int inductivePin_1 = 15;  // Pin connected to the inductive sensor
const int inductivePin_2 = 2;  // Pin connected to the inductive sensor
const int inductivePin_3 = 4;  // Pin connected to the inductive sensor
...
// const int trigPin4 = ;  // Trigger pin for sensor 4
...
void loop() {
  ...
  count_arr[0] += read_from_pin_ultrasonic(ultrasonic1) || read_from_pin(inductivePin_1);
  count_arr[1] += read_from_pin_ultrasonic(ultrasonic2) || read_from_pin(inductivePin_2);
  count_arr[2] += read_from_pin_ultrasonic(ultrasonic3) || read_from_pin(inductivePin_3);
  if(type == 1)
    // count_arr[3] += read_from_pin_ultrasonic(ultrasonic4);
  ...
  Serial.println();
  // END DEBUG PRINTS
  write_to_om2m(type, count_arr, arr2, "Junction-5/Data");
...
int read_from_pin_ultrasonic (Ultrasonic sensor) {
